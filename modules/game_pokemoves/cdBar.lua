...
function init()
   ...
   ProtocolGame.registerExtendedOpcode(95, 
...
function terminate()
   ...
   ProtocolGame.unregisterExtendedOpcode(95)
   destroyIcons()
   cdBarWin:destroy()
end
...
function getParams(buffer)
if not g_game.isOnline() then return end
	if string.find(buffer, '12//,') then
		if string.find(buffer, 'hide') then hide() else show() end
...
if not cdBarWin:isVisible() then return end 
   ...
   local pathOn = "/images/game/pokedex/moves_icon/"..progress:getTooltip().."_on.png"
