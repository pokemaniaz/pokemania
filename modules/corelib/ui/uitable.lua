...
function UITable:sort()
  ...
  if self.sortType == TABLE_SORTING_ASC then
    table.sort(self.rows, function(rowA, b)
	  if rowA:getChildByIndex(self.sortColumn).sortvalue == "Offer" then
		rowA:getChildByIndex(self.sortColumn).sortvalue = 0
	  end
	  if b:getChildByIndex(self.sortColumn).sortvalue == "Offer" then
		b:getChildByIndex(self.sortColumn).sortvalue = 0 
	  end
