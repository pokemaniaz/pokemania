...
function init()
  connect(g_game, { onGameEnd = onGameEnd })
	  ProtocolGame.registerExtendedOpcode(125, function(protocol, opcode, buffer) doHide(buffer) doCountPlayer(buffer) closeChanneGravando(buffer) doRequestPassword(buffer) openCreate(buffer) doGravando(buffer) checkListChannels(buffer)  doAssistindo(buffer) end)
...
function Send()
	local msg = donateWindow:getChildById('MsgText'):getText()
	local rubys = donateWindow:getChildById('useDiamond'):isChecked()
	local qnt = donateWindow:getChildById('QntText'):getText()
	g_game.getProtocolGame():sendExtendedOpcode(125, "donate/" .. msg .."/" .. tostring(rubys).. "/" .. qnt .. "/" .. playerName)
end
...
function doGravando(buffer)
	...
	deadWindow:getChildById("donate"):setVisible(false)
	gravando = true
	deadWindow:show()
	reset = false
	countTime()
...
function doHide(buffer)
	if not buffer:find("hide") then return true end
	hide(1)
end

function doAssistindo(buffer)
	if not buffer:find("assistir") then return true end
	hide(2)
	deadWindow = g_ui.displayUI('tvcam')
	local strings = buffer:explode(":")
	channelName = strings[2],
	deadWindow:getChildById("tvCamLabel"):setText("Assitindo...")
	deadWindow:getChildById("tvCamNameLabel"):setText(channelName .. " - 00:00:00 (" .. users .. ")")
	deadWindow:getChildById("proximo"):setVisible(true)
	deadWindow:getChildById("anterior"):setVisible(true)
	deadWindow:getChildById("donate"):setVisible(true)
	deadWindow:show()
	gravando = false
	reset = false
	countTime()
...
function closeChanne()
    ...
    if gravando == true then
        ...
        g_game.getProtocolGame():sendExtendedOpcode(125, "closeView/")
    end
end
...
function Back()
	g_game.getProtocolGame():sendExtendedOpcode(125, "back/")
end

function Next()
	g_game.getProtocolGame():sendExtendedOpcode(125, "next/")
end
		...
		for i = 2, #channels do
			if #channels[i]:explode("/") > 0 then 
				...
				local nameWidget = widget:getChildById("name")
					  nameWidget:setText(channels[i]:explode("/")[1] .. "/" .. channels[i]:explode("/")[2] .. "." )
...
function doWatch()
  ...
  if selected then
	...
	playerName = selected:getChildById("name"):getText():explode("/")[1]
  else
