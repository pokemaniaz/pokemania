...
function init()
  ...
  disableChatBox = consolePanel:getChildById('disableChatCheckBox')
  lockChannelBox = consolePanel:getChildById('lockConsolePanel')
  ...
  g_keyboard.bindKeyDown('Enter', visibleChat, modules.game_interface.getBottomPanel())
  g_keyboard.bindKeyDown('Enter', enableChat, consolePanel)
...
function visibleChat()
  if consolePanel:isVisible() then return end
  consolePanel:setVisible(true)
  enableChat()
end

function enableChat()
  if not consolePanel:isVisible() then return false end
  consolePanel:focus()
  consoleMode = 1
  disableChatBox:setVisible(true)
  consolePanel:getChildById('sayModeButton'):setVisible(true)
  ...
  if visibleEvent then
    removeEvent(visibleEvent)
  end
  if consolePanel:getOpacity() ~= 1 then
    --addEvent(function() g_effects.fadeIn(consolePanel, 0) end)
	showConsolePanel()
  end
...
function disableChat()
  if disableChatBox:isChecked() then return end
  disableChatBox:setVisible(false)
  consoleMode = 0
  consolePanel:getChildById('sayModeButton'):setVisible(false)
  local gameInterface = modules.game_interface
  consoleTextEdit:setVisible(false)
  event = scheduleEvent(function() hideConsolePanel() end, 10000)
...
function hideConsolePanel()
  if g_game.isOnline() then
    addEvent(function() g_effects.fadeOut(consolePanel, 250) end)
    visibleEvent = scheduleEvent(function() consolePanel:setVisible(false) end, 250)
  end
end

function showConsolePanel()
  if g_game.isOnline() then
    addEvent(function() g_effects.fadeIn(consolePanel, 250) end)
    visibleEvent = scheduleEvent(function() consolePanel:setVisible(true) end, 250)
  end
end
...
function save()
  ...
  settings.lockConsolePanel = consolePanel:getChildById('lockConsolePanel'):isChecked()
  settings.disableConsolePanel = disableChatBox:isChecked()
  settings.bottomPanelPos = modules.game_interface.getMoveBottomPanel():getPosition()
...
function load()
  ...
  if settings then
    ...
    consolePanel:getChildById('lockConsolePanel'):setChecked(settings.lockConsolePanel)
    if settings.disableConsolePanel then
      disableChatBox:setChecked(true)
      consolePanel:setVisible(true)
      consolePanel:focus()
      local gameInterface = modules.game_interface
      gameInterface.unbindWalkKey('W', North)
      gameInterface.unbindWalkKey('E', NorthEast)
      gameInterface.unbindWalkKey('D', East)
      gameInterface.unbindWalkKey('C', SouthEast)
      gameInterface.unbindWalkKey('S', South)
      gameInterface.unbindWalkKey('Z', SouthWest)
      gameInterface.unbindWalkKey('A', West)
      gameInterface.unbindWalkKey('Q', NorthWest)
      g_keyboard.unbindKeyDown('Enter', consolePanel)
      g_keyboard.bindKeyDown('Enter', sendCurrentMessage, consolePanel)
    end
    if settings.bottomPanelPos then
      modules.game_interface.getMoveBottomPanel():setPosition(settings.bottomPanelPos)
    else
      modules.game_interface.getMoveBottomPanel():setPosition(topoint('50 523'))
    end
  end
...
function sendCurrentMessage()
  ...
  if #message == 0 then disableChat() return end
  consoleTextEdit:clearText()
