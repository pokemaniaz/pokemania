...
function init()
  ...
  connect(g_game, 'onTextMessage', isMyPokemon)
...
function terminate()
  ...
  disconnect(g_game, 'onTextMessage', isMyPokemon)
...
function getFollowingPokemon()
  if following == 'pokemon' then
    return true
  else
    return false
  end
...
function setFollowingPokemon(value)
  if value then
    following = 'pokemon'
  else
    following = 'player'
...
function isMyPokemon(mode, text)
  if not getCodeBuffer(mode, 156, text) then return end
  local buffer = getCodeBuffer(mode, 156, text)
  pokemonId = tonumber(buffer)
  modules.game_battle.reallocateMyPokemon(pokemonId)
end
...
function onLockConsolePanelCheckChange(value)
  if value then
    gameRootPanel:recursiveGetChildById('moveBottomPanel'):setVisible(false)
  else
    gameRootPanel:recursiveGetChildById('moveBottomPanel'):setVisible(true)
  end
end
...
function onUseWith(clickedWidget, mousePosition)
  ...
  elseif clickedWidget:getClassName() == 'UIButton' then
    for i = 1, 6 do
      if clickedWidget:getId() == 'icon'..i then
...
function createThingMenu(menuPosition, lookThing, useThing, creatureThing)
  ...
  if lookThing and lookThing:isPlayer() then
  ...
  if useThing then
    if useThing:isContainer() then
      ...
      if useThing:isMultiUse() then
		...
		if useThing:getId() >= 19840 and useThing:getId() <= 19849 then
			menu:addOption(tr('Take Pokeball'), function() g_game.use(useThing) end, shortcut)
		else
  ...
  if creatureThing then
    ...
    local pokemonName = getPokemonNameByOutfit(creatureThing:getOutfit().type)
    ...
    if creatureThing:isLocalPlayer() then
      ...
      if following == 'pokemon' then
        menu:addOption(tr('Control Player'), function() gameMapPanel:followCreature(creatureThing) following = 'player' end)
      end
      ...
      if creatureThing:isPartyMember() then
        if creatureThing:isPartyLeader() then
          if creatureThing:isPartySharedExperienceActive() then
            menu:addOption(tr('Disable Shared Experience'), function() g_game.getProtocolGame():sendExtendedOpcode(151, 'disable') end)
          else
            menu:addOption(tr('Enable Shared Experience'), function() g_game.getProtocolGame():sendExtendedOpcode(151, 'enable') end)
          end
        end
      ...
      if creatureThing:getPosition().z == localPosition.z then
        ...
        if creatureThing:getId() == pokemonId and following == 'player' then
          menu:addOption(tr('Control Pokemon'), function() gameMapPanel:followCreature(creatureThing) following = 'pokemon' end)
		end
      end
...
function setupViewMode(mode)
  ...
  gameMapPanel:setZoom(14) -- Aqui eu botei 15, e nas sources eu coloquei X 20 Y 20, Edite como quiser, � o zoom do client
