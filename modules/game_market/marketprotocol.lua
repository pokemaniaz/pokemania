...
local function readMarketOffer(msg, action, var)
  ...
  return MarketOffer.new({timestamp, counter}, action, Item.create(itemId), amount, price, playerName, itemName, state, var)
end
...
local function parseMarketItemDesc(protocol, msg)
  local Desc = msg:getString()
  local itemId = msg:getU16()
  signalcall(Market.onMarketItemDesc, Desc, itemId)
  return true
end
...
function MarketProtocol.registerProtocol()
  ...
  ProtocolGame.registerOpcode(GameServerOpcodes.GameServerMarketItemDesc, parseMarketItemDesc)
...
function MarketProtocol.unregisterProtocol()
  ...
  ProtocolGame.unregisterOpcode(GameServerOpcodes.GameServerMarketItemDesc, parseMarketItemDesc)
...
function MarketProtocol.sendMarketItemDesc(timestamp, counter)
    local msg = OutputMessage.create()
    msg:addU8(GameServerOpcodes.GameServerMarketItemDesc)
...
function MarketProtocol.sendMarketBrowseMyHist()
  MarketProtocol.sendMarketBrowse(MarketRequest.MyHistory)
end

function MarketProtocol.sendMarketCreateOffer(type, pos, spriteId, stackpos, amount, price, anonymous)
  if g_game.getFeature(GamePlayerMarket) then
    local msg = OutputMessage.create()
