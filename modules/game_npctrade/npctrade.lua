...
--    weightLabel:setText(string.format('%.2f', selectedItem.weight*quantity) .. ' ' .. WEIGHT_UNIT)
    priceLabel:setText(formatCurrency(getItemPrice(selectedItem)/100))
  end
end
...
function canTradeItem(item)
  if getCurrentTradeType() == BUY then
    return true --(ignoreCapacity:isChecked() or (not ignoreCapacity:isChecked() and playerFreeCapacity >= item.weight)) and playerMoney >= getItemPrice(item, true)
  else
...
function refreshItem(item)
 ...
 -- weightLabel:setText(string.format('%.2f', item.weight) .. ' ' .. WEIGHT_UNIT)
  priceLabel:setText(formatCurrency(getItemPrice(item)/100))
...
function refreshTradeItems()
  ...
  for key,item in pairs(currentTradeItems) do
    ...
    local price = formatCurrency(item.price/100)
    text = text .. '\n' .. price
...
function refreshPlayerGoods()
  ...
  moneyLabel:setText(formatCurrency(playerMoney/100))
...
function formatCurrency(amount)
  if CURRENCY_DECIMAL then
    return CURRENCY .. '' .. string.format("%.02f", amount/100.0)
  else
